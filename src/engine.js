...
export const questionsFor = session => session?.mode === 'skull-theory' ? skullTheoryQuestions : session?.mode === 'muscle-details' ? detailQuestions : session?.mode === 'muscle-visual' ? muscleVisualQuestions : session?.mode === 'skull' ? skullQuestions : muscleQuestions;
...
const versionFor = mode => mode === 'skull-theory' ? SKULL_THEORY_VERSION : mode === 'muscle-details' ? DETAIL_VERSION : mode === 'muscle-visual' ? MUSCLE_VISUAL_VERSION : mode === 'skull' ? 'skull-v1' : BANK_VERSION;
...
export function validSession(value) {
  ...
  if (!value || value.version !== versionFor(value.mode) || (value.mode !== undefined && !['muscles', 'skull', 'muscle-visual', 'muscle-details', 'skull-theory'].includes(value.mode)) || !['quiz', 'results', 'review'].includes(value.screen)) return false;
...
export function readSession(storage, key = STORAGE_KEY) {
  try {
    ...
    if (!validSession(value) || key !== (value.mode === 'skull-theory' ? SKULL_THEORY_STORAGE_KEY : value.mode === 'muscle-details' ? DETAIL_STORAGE_KEY : value.mode === 'muscle-visual' ? MUSCLE_VISUAL_STORAGE_KEY : value.mode === 'skull' ? SKULL_STORAGE_KEY : STORAGE_KEY)) return { session: null, warning: 'O progresso salvo está inválido ou é de outra versão. Inicie uma nova sessão.' };
