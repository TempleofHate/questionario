...
export function visualImage(q, answer, review = false) {
  ...
  return `<figure class="visual-question">
    ...
    <div class="image-scroll" ${review ? '' : 'tabindex="0" aria-label="Área da fotografia; role para explorar a imagem ampliada"'}><div class="image-plane" style="aspect-ratio:${image.width}/${image.height};--image-ratio:${image.width / image.height}">
      <img src="${e(image.local)}" width="${image.width}" height="${image.height}" alt="Fotografia de peça óssea do crânio para identificação anatômica" ${review ? 'loading="lazy"' : 'fetchpriority="high"'} draggable="false">
      <svg class="hotspot-layer" viewBox="0 0 100 100" preserveAspectRatio="none" ${!answer && !review ? 'data-hotspot tabindex="0" role="button" aria-label="Responder na fotografia: use as setas para mover o cursor, Shift para precisão e Enter para responder" aria-describedby="visual-help"' : 'role="img" aria-label="Localização correta destacada"'}>
    ...
    <figcaption>Fotografia: MAKY.OREL · <a href="https://creativecommons.org/licenses/by-sa/4.0/" target="_blank" rel="noopener">CC BY-SA 4.0</a> · Wikimedia Commons. Redimensionada e comprimida.${answer ? '<br>Contorno verde: região correta. A cruz indica seu clique, quando houve resposta.' : ''}</figcaption>
...
export function modeNav(mode) {
  return `<nav class="mode-nav" aria-label="Modo de estudo"><button class="text-button" data-action="mode" data-mode="muscles" ${mode === 'muscles' ? 'aria-current="page"' : ''}>Músculos · 150 questões</button><button class="text-button" data-action="mode" data-mode="skull" ${mode === 'skull' ? 'aria-current="page"' : ''}>Crânio visual · 70 questões</button></nav>`;
}
