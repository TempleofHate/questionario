...
export function modeNav(mode) {
  return `<nav class="mode-nav" aria-label="Modo de estudo"><button class="text-button" data-action="mode" data-mode="muscles" ${mode === 'muscles' ? 'aria-current="page"' : ''}>Músculos · 150 questões</button><button class="text-button" data-action="mode" data-mode="skull" ${mode === 'skull' ? 'aria-current="page"' : ''}>Crânio visual · 70 questões</button><button class="text-button" data-action="mode" data-mode="muscle-visual" ${mode === 'muscle-visual' ? 'aria-current="page"' : ''}>Músculos visuais · 70 questões</button></nav>`;
}
