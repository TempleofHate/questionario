import { craniometriaGroups, CRANIOMETRIA_PDF_URL } from './data/craniometria.js';
import { skullTheoryGroups, skullNoteSources } from './data/skull-theory.js';
...
const source = q => q.source.image ? `<a href="${e(q.source.url)}" target="_blank" rel="noopener">Conferir nas anotações · imagem ${q.source.image}<span class="sr-only"> (abre em outra aba)</span></a>` : q.type === 'hotspot' ? `<a href="${e(q.source.url)}" target="_blank" rel="noopener">Imagem original no ${e(q.image.provider || 'Wikimedia Commons')}</a> · <a href="${q.credits || "/docs/skull-sources.html"}">Fontes e créditos</a>` : `<a href="${e(q.source.url || PDF_URL)}#page=${q.source.page}" target="_blank" rel="noopener">Conferir no PDF · p. ${q.source.page}<span class="sr-only"> (abre em outra aba)</span></a>`;
export function sidebar(session, mode = 'muscles') {
  const bank = questionsFor(session || { mode });
  const topics = mode === 'craniometria' ? craniometriaGroups : mode === 'skull-theory' ? skullTheoryGroups : mode === 'muscle-details' ? detailGroups : mode === 'muscle-visual' ? [['Anatomia visual muscular', bank]] : mode === 'skull' ? [['Anatomia visual do crânio', bank]] : groups;
  ...
  return `<aside class="sidebar" aria-label="Assuntos do quiz"><h2>${session ? 'Sua sessão' : 'Roteiro de estudo'}</h2>
    ...
    }).join('')}</ol><div class="session-notes"><p>${bank.length} questões<br>1 minuto por questão</p>${session ? `<p>${stats.correct} ${stats.correct === 1 ? 'acerto' : 'acertos'} até aqui</p><button class="text-button" data-action="reset">Apagar progresso e recomeçar</button>` : isVisual({ mode }) ? '<p>Imagens anatômicas.<br>Resposta por toque na imagem.</p>' : mode === 'craniometria' ? '<p>Base única: o PDF<br>de Craniometria.</p>' : mode === 'skull-theory' ? '<p>Base única: duas imagens<br>de anotações sobre crânio.</p>' : '<p>Base única: o PDF<br>de cabeça e pescoço.</p>'}</div></aside>`;
}
export function intro(mode = 'muscles') {
  if (mode === 'muscle-visual') return muscleVisualIntro();
  if (mode === 'skull') return visualIntro();
  return `<section class="intro"><h1 id="screen-title" tabindex="-1">${mode === 'craniometria' ? 'Craniometria' : mode === 'skull-theory' ? 'Crânio — Fossas, Forames e Resistência' : mode === 'muscle-details' ? 'Estude cada músculo.<br>Relacione os quatro aspectos.' : 'Conheça os músculos.<br>Entenda as relações.'}</h1>
    <p class="lead">${mode === 'craniometria' ? '40 questões de múltipla escolha sobre conceitos, dimorfismo sexual, aspectos etários, medidas, índices, Plano de Frankfurt e pontos craniométricos, baseadas exclusivamente no PDF de Craniometria.' : mode === 'skull-theory' ? '40 questões de múltipla escolha sobre envelhecimento, resistência, fossas, passagens e relações anatômicas do crânio, baseadas exclusivamente nas duas imagens de anotações.' : mode === 'muscle-details' ? 'Uma questão por músculo do PDF, reunindo origem, inserção, função e inervação. Escolha a associação correta dos quatro aspectos. Quando a fonte não informa a inervação, essa ausência é indicada.' : 'Uma sessão de estudo sobre os músculos da cabeça e do pescoço, do primeiro conceito às relações entre origem, inserção, ação e inervação.'}</p>
    <dl class="intro-facts"><div><dt>Questões</dt><dd>${questionsFor({ mode }).length}</dd></div><div><dt>Por questão</dt><dd>60 segundos</dd></div><div><dt>Fonte</dt><dd>${mode === 'craniometria' ? 'PDF de Craniometria · 77 páginas' : mode === 'skull-theory' ? '2 imagens de anotações' : '7 páginas de estudo'}</dd></div></dl>
    ...
    <div class="actions"><button class="primary" data-action="start">Iniciar quiz ${arrow}</button>${mode === 'skull-theory' ? skullNoteSources.map((url, i) => `<a href="${e(url)}" target="_blank" rel="noopener">Ler anotações · imagem ${i + 1}<span class="sr-only"> (abre em outra aba)</span></a>`).join('') : `<a href="${e(mode === 'craniometria' ? CRANIOMETRIA_PDF_URL : PDF_URL)}" target="_blank" rel="noopener">Ler material de estudo<span class="sr-only"> (abre em outra aba)</span></a>`}</div>
