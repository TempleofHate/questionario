import { questions, groups, PDF_URL } from './data/questions.js';
import { summary, remainingSeconds } from './engine.js';
...
const source = q => `<a href="${e(PDF_URL)}#page=${q.source.page}" target="_blank" rel="noopener">Conferir no PDF · p. ${q.source.page}<span class="sr-only"> (abre em outra aba)</span></a>`;
export function sidebar(session) {
  ...
  return `<aside class="sidebar" aria-label="Assuntos do quiz"><h2>${session ? 'Sua sessão' : 'Roteiro de estudo'}</h2>
    <ol class="topic-list">${groups.map(([topic, rows]) => {
      const active = session?.screen === 'quiz' && questions[session.index].topic === topic;
    ...
    }).join('')}</ol><div class="session-notes"><p>150 questões<br>1 minuto por questão</p>${session ? `<p>${stats.correct} ${stats.correct === 1 ? 'acerto' : 'acertos'} até aqui</p><button class="text-button" data-action="reset">Apagar progresso e recomeçar</button>` : '<p>Base única: o PDF<br>de cabeça e pescoço.</p>'}</div></aside>`;
}
...
function timeoutNotice(session) {
  ...
  const q = questions[session.notice];
...
export function quiz(session) {
  ...
  return `${timeoutNotice(session)}<section class="question" aria-labelledby="screen-title">
    ...
    <form id="answer-form"><fieldset ${answer ? 'disabled' : ''}><legend class="sr-only">Selecione uma alternativa</legend><div class="options">${q.options.map((option, i) => {
    ...
    <div class="question-actions"><p>${answer ? 'Leia a justificativa. Avance quando estiver pronto.' : 'Selecione uma alternativa e confirme.'}</p>${answer ? `<button type="button" class="primary" data-action="next">${session.index === 149 ? 'Ver resultado' : 'Próxima questão'} ${arrow}</button>` : `<button type="submit" class="primary" id="confirm" ${session.selected === null ? 'disabled' : ''}>Confirmar resposta</button>`}</div></form></section>`;
}
export function results(session) {
  const stats = summary(session);
  return `${timeoutNotice(session)}<section class="results"><h1 id="screen-title" tabindex="-1">Seu estudo, em perspectiva.</h1><p class="lead">Você concluiu as 150 questões. Use o resultado para orientar a próxima revisão.</p>
    <dl class="result-summary"><div class="grade"><dt>Nota final</dt><dd>${decimal(stats.grade)}<span> / 10</span></dd></div><div><dt>Aproveitamento</dt><dd>${decimal(stats.percent)}%</dd></div><div><dt>Acertos</dt><dd>${stats.correct}<span> / 150</span></dd></div></dl>
    ...
    <div class="actions"><button class="primary" data-action="review">Revisar respostas ${arrow}</button><button class="secondary" data-action="reset">Refazer quiz</button></div><p class="fineprint">Nota = acertos ÷ 150 × 10. O tempo soma os segundos usados para responder, sem incluir a leitura do feedback.</p></section>`;
}
export function review(session, filter = 'all') {
  ...
  return `<section class="review"><div class="review-top"><h1 id="screen-title" tabindex="-1">Revisar respostas</h1><button class="text-button" data-action="results">Voltar ao resultado</button></div><p class="lead">Releia as explicações e consulte o trecho correspondente no material.</p><div class="review-controls"><label for="review-filter">Mostrar</label><select id="review-filter"><option value="all" ${filter === 'all' ? 'selected' : ''}>Todas as respostas</option><option value="wrong" ${filter === 'wrong' ? 'selected' : ''}>Somente erros</option><option value="timeout" ${filter === 'timeout' ? 'selected' : ''}>Tempo esgotado</option></select><span class="muted">${rows.length} ${rows.length === 1 ? 'questão' : 'questões'}</span></div>
    <div class="review-list">${rows.length ? rows.map(({ q, i, answer }) => {
      const right = !answer.timedOut && answer.choice === q.answerIndex;
      return `<article class="review-item"><div class="review-meta"><span>Questão ${i + 1} · ${e(q.topic)}</span><strong class="${right ? 'positive' : 'negative'}">${answer.timedOut ? 'Tempo esgotado' : right ? 'Correta' : 'Incorreta'}</strong></div><h2>${e(q.prompt)}</h2><dl><div><dt>Sua resposta</dt><dd>${answer.choice === null ? 'Não respondida — tempo esgotado' : e(q.options[answer.choice])}</dd></div><div><dt>Resposta correta</dt><dd class="positive">${e(q.options[q.answerIndex])}</dd></div></dl><div class="justification"><h3>Justificativa:</h3><p>${e(q.explanation)}</p></div>${source(q)}</article>`;
