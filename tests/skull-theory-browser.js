...
import { skullTheoryQuestions as detailQuestions } from '../src/data/skull-theory.js';
...
try {
  ...
  await page.locator('[data-mode="skull-theory"]').click();
  assert.match(await page.locator('#main').innerText(), /40/);
  await page.screenshot({ path: '.impeccable/review/skull-theory-intro-desktop.png', fullPage: true });
  await page.getByRole('button', { name: 'Iniciar quiz', exact: true }).click();
  ...
  for (const width of [1280, 390, 320]) {
    ...
    await page.screenshot({ path: `.impeccable/review/skull-theory-question-${width}.png`, fullPage: true });
  }
  ...
  await page.reload();
  assert.equal(await page.locator('.review-item').count(), 40);
  for (const mode of ['skull', 'muscle-visual', 'muscle-details', 'skull-theory']) {
    await page.locator(`[data-mode="${mode}"]`).click();
    assert.equal(await page.locator(`[data-mode="${mode}"]`).getAttribute('aria-current'), 'page');
  }
  assert.deepEqual(errors, []);
  console.log('OK: 40 questões, revisão, recarga, cinco partes independentes e larguras 1280/390/320.');
