...
import { STORAGE_KEY, MUSCLE_VISUAL_STORAGE_KEY } from '../src/engine.js';
...
try {
  for (const mobile of [false,true]) {
    ...
    await page.locator('[data-hotspot]').click({position:{x:5,y:5}});
    const skullLegacy=await page.evaluate(()=>localStorage.getItem('musculos-em-estudo:skull-session'));
    ...
    await page.getByRole('button',{name:'Iniciar quiz visual',exact:true}).click();
    const saved=()=>page.evaluate(k=>JSON.parse(localStorage.getItem(k)),MUSCLE_VISUAL_STORAGE_KEY);
    for (let i=0;i<70;i++) {
      const q=muscleVisualQuestions[i];
      await page.locator('.image-plane img').evaluate(img=>img.decode());
      ...
      if(i===0)await page.screenshot({path:`.impeccable/review/muscle-visual-${mobile?'mobile':'desktop'}.png`,fullPage:true});
      ...
      if(i<3) {
        await page.screenshot({path:`.impeccable/review/muscle-visual-${type}-${mobile?'mobile':'desktop'}.png`,fullPage:true});
        await page.reload();
  ...
  await writeFile('.impeccable/review/muscle-visual-browser-report.json',JSON.stringify({passed:true,questions:70,viewports:[1440,390],correct:24,errors:46,timeouts:23,localImages:Object.keys(muscleVisualImages).length,legacyProgressPreserved:true,consoleErrors:errors},null,2));
